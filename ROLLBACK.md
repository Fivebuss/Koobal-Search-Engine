...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.1-beta** | `0.8.5.1` / `0.8.5.1` | `0.8.5` (+BUILD 1) | **Current packaged beta — dual-deployed to main + ModTest.** Rollback baseline remains v0.8.4.0. | Same assembly line as v0.8.5.1, packaged as forum beta zip `KoobalSearchEngine_v0.8.5.1-beta.zip`. **Suggestion quality rebalance:** restore categorizer/metadata above parts (`RankScore` 100+ for parts); budget high-value FilterFunction/Category/Manufacturer/Diameter (8) vs token-style Tag/Module/Resource/Tech (3); denylist synonym junk (`space`/`speed`/`spoil`/…); no `resourceInfo` prose scraping; title-first part scoring for Spark-like hits. Annotated tag **`v0.8.5.1-beta`**. Author `timbrwolf1121`. |
| **v0.8.5.1** | `0.8.5.1` / `0.8.5.1` | `0.8.5` | **Code lineage / intermediate tag (superseded as “current deploy” by v0.8.5.1-beta package).** | **FIX + QoL under one label.** (1) **SearchStart NRE:** Harmony Prefix skip of `SearchRoutine` returned null IEnumerator → stock `SearchStart` → `StartCoroutine(null)` ("routine is null"). Fix: do not skip `SearchRoutine`; block void `SearchStart` only while apply-suppressed; clear custom-filter guard on stock `SearchStart` / focus-typing; `RecoverAfterFailedApply` on ApplySuggestion catch. (2) **Hangar-free index load:** `GameLoadBootstrap` sole builder; `EditorSearchHook` wait/UI-hook only; no hangar `BuildIfNeeded`; clear stale build lock after scene-host destroy; `BuildIfNeeded`/`background` builders use try/finally so locks/completions cannot stick. (3) **1080p readability pass:** larger primary/subtitle/header fonts + brighter contrast colors; branding footer captions slightly larger; named typography constants in `SearchDropdownPanel` / `KoobalWordmarkBuilder`. (4) **Branding footer centering:** empty-query Koobal wordmark no longer drifts left — stretch/center anchors, footer+content MiddleCenter VLGs, DestroyImmediate on rebuild, force layout rebuild after show/width change. (5) **Clicked-suggestion history (additive):** `ApplySuggestion` records display text via `RememberClickedSuggestion` *before* stock apply (so apply exceptions cannot drop it); typed Enter/submit `CommitSearchHistory` path unchanged. (6) **Clear-history trashcan icon:** programmatic 16x16 `Texture2D` sprite (LiberationSans lacks U+1F5D1; no stock Squad trash UI sprite). (7) **Suggestion quality rebalance:** parts stay below categorizer/metadata; token-style FilterTag/etc. capped; synonym junk denylisted; title-first part scoring. **Note:** hangar alignment was briefly labeled **v0.8.5.2** on ModTest; that label is folded back into **v0.8.5.1** / **v0.8.5.1-beta**. Local `v0.8.5.2` tag/zip removed to avoid confusion. |
| **v0.8.5.2** | *(temporary label only)* | — | **Folded into v0.8.5.1 — do not use.** | Temporary ModTest label for hangar-free index load only. Code retained under **v0.8.5.1**; local tag + `KoobalSearchEngine_v0.8.5.2.zip` removed. |
...
As of **v0.8.5.1-beta**, both installs run the same sanitary GameData package
(assembly `0.8.5.1`). The verified-safe rollback baseline remains **v0.8.4.0**.

| Install | Version | Path |
|---------|---------|------|
| Main KSP | **v0.8.5.1-beta** (`0.8.5.1`) | `GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |
| ModTest | **v0.8.5.1-beta** (`0.8.5.1`) | `Kerbal Space Program - ModTest/GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |

Forum / Desktop package: `KoobalSearchEngine_v0.8.5.1-beta.zip`
...
- `v0.8.5.1-beta` — annotated release tag for the packaged beta (prefer this).
- `v0.8.5.1` — earlier annotated tag on the same version lineage (kept; do not delete).

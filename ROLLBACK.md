...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.1** | `0.8.5.1` / `0.8.5.1` | `0.8.5` | **Current ModTest deploy (running test). Rollback baseline remains v0.8.4.0. Beta 0.8.5.0 artifacts left intact.** | **FIX — recoverable SearchStart NRE after ApplySuggestion.** Harmony Prefix skip of `SearchRoutine` returned null IEnumerator → stock `SearchStart` → `StartCoroutine(null)` ("routine is null"). Typing after apply crashed until blur/refocus. Fix: do not skip `SearchRoutine`; block void `SearchStart` only while apply-suppressed; clear custom-filter guard on stock `SearchStart` / focus-typing; `RecoverAfterFailedApply` on ApplySuggestion catch. |
...
| ModTest | **v0.8.5.1** (SearchStart NRE fix; rollback baseline v0.8.4.0) | `Kerbal Space Program - ModTest/GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |

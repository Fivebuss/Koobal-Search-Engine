...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.2** | `0.8.5.2` / `0.8.5.2` | `0.8.5` (+BUILD 2) | **Current — dual-deployed to main + ModTest (test-report line).** Rollback baseline remains v0.8.4.0. | **Branding footer centering (root layout).** Empty-query Koobal wordmark stays centered with history rows present and after dropdown resize/reposition. Root cause: nested footer LayoutGroup + stretch anchors fought TMP preferred width so the host collapsed to text width and left-aligned. Fix: stretch-fill branding host (no LayoutGroup on footer); intrinsic-width wordmark/captions via ContentSizeFitter; parent VLG MiddleCenter without force-expand; re-assert stretch after panel size changes. Suggestion logic unchanged. PostV1 still gated off. Annotated tag **`v0.8.5.2`**. Author `timbrwolf1121`. (Reclaims the version number previously used briefly for hangar-free load, which was folded into v0.8.5.1.) |
...
| **v0.8.5.1** | `0.8.5.1` / `0.8.5.1` | `0.8.5` | **Code lineage / intermediate tag (superseded as “current deploy” by v0.8.5.2).** | **FIX + QoL under one label.** (1) **SearchStart NRE:** Harmony Prefix skip of `SearchRoutine` returned null IEnumerator → stock `SearchStart` → `StartCoroutine(null)` ("routine is null"). Fix: do not skip `SearchRoutine`; block void `SearchStart` only while apply-suppressed; clear custom-filter guard on stock `SearchStart` / focus-typing; `RecoverAfterFailedApply` on ApplySuggestion catch. (2) **Hangar-free index load:** `GameLoadBootstrap` sole builder; `EditorSearchHook` wait/UI-hook only; no hangar `BuildIfNeeded`; clear stale build lock after scene-host destroy; `BuildIfNeeded`/`background` builders use try/finally so locks/completions cannot stick. (3) **1080p readability pass:** larger primary/subtitle/header fonts + brighter contrast colors; branding footer captions slightly larger; named typography constants in `SearchDropdownPanel` / `KoobalWordmarkBuilder`. (4) **Branding footer centering (partial):** attempted stretch/center anchors + nested VLGs + ForceRebuild — still left-drifted with history / dropdown move-down; fixed properly in **v0.8.5.2**. (5) **Clicked-suggestion history (additive):** `ApplySuggestion` records display text via `RememberClickedSuggestion` *before* stock apply (so apply exceptions cannot drop it); typed Enter/submit `CommitSearchHistory` path unchanged. (6) **Clear-history trashcan icon:** programmatic 16x16 `Texture2D` sprite (LiberationSans lacks U+1F5D1; no stock Squad trash UI sprite). (7) **Suggestion quality rebalance:** parts stay below categorizer/metadata; flat top-8 categorizer by RankScore; synonym junk denylisted; query-length-aware part scoring (≤2 title-first / ≥3 tag-weighted). **Note:** hangar alignment was briefly labeled **v0.8.5.2** on ModTest then folded into **v0.8.5.1**; that number is now reused for the branding-centering test-report line. |
...
- `v0.8.5.2` — annotated release tag for the branding-centering test-report line (prefer this).

...
| 3 | **Desktop KSE stable zip** | `C:\Users\timbr\Desktop\KSE\KoobalSearchEngine_v0.8.4.0_CORE_STABLE.zip` | unzip into KSP `GameData/` |
...
|-------|-------------------|----------------|--------|-------|
| **v0.8.6.0** | `0.8.6.0` / `0.8.6.0` | `0.8.6` | **Current — dual-deployed to main + ModTest.** Rollback baseline remains v0.8.4.0. | **Suggestion research / quality beta.** Soft popularity prior (`PartPopularity.cfg`), personal history prior, type-aware filter slot reserve, disambiguation subtitles, history-as-suggestions, weak-result UX, mod-suite duplicate-title preference, denylist tighten; organic apply guard verified. **0.8.5.x stays open for hotfixes.** Annotated tag **`v0.8.6.0`**. Author `timbrwolf1121`. |
| **v0.8.5.2** | `0.8.5.2` / `0.8.5.2` | `0.8.5` (+BUILD 2) | **Hotfix/stability line (superseded as current by v0.8.6.0).** Rollback baseline remains v0.8.4.0. | **Branding footer centering** plus **organic apply race restore (same assembly, no version bump).** (1) Empty-query wordmark stays centered with history/resize. (2) `StockSearchGuard` again blocks void `SearchStart` while a Koobal custom filter is active (lost v0.7 / over-cleared in v0.8.5.1) so `ApplyPrecisePart` / categorizer apply survive blur instead of loose stock overwrite; typing still clears via `CancelPendingStockSearchForTyping`; never skip `SearchRoutine`. Annotated tag **`v0.8.5.2`**. Author `timbrwolf1121`. |
...
As of **v0.8.6.0**, both installs run the same GameData package
(assembly `0.8.6.0`). The verified-safe rollback baseline remains **v0.8.4.0**.
The **0.8.5.x** line remains available for hotfixes without consuming suggestion-research work.

| Install | Version | Path |
|---------|---------|------|
| Main KSP | **v0.8.6.0** (`0.8.6.0`) | `GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |
| ModTest | **v0.8.6.0** (`0.8.6.0`) | `Kerbal Space Program - ModTest/GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |

Forum / Desktop KSE package: `C:\Users\timbr\Desktop\KSE\KoobalSearchEngine_v0.8.6.0.zip`
(also in `Source/PartSearchSuggest/ReleaseArchive/`).

Stable baseline archive: `Desktop/KSE/KoobalSearchEngine_v0.8.4.0_CORE_STABLE.zip` (CKAN-style
...
**Emergency stock-safe fallback:** `Desktop/KSE/KoobalSearchEngine_v0.8.3.17-fallback_STABLE_FALLBACK.zip`

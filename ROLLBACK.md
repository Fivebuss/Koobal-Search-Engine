...
|-------|-------------------|----------------|--------|-------|
| **v0.8.5.1** | `0.8.5.1` / `0.8.5.1` | `0.8.5` | **Current ModTest deploy (combined). Rollback baseline remains v0.8.4.0. Beta 0.8.5.0 artifacts left intact.** | **FIX + QoL under one label.** (1) **SearchStart NRE:** Harmony Prefix skip of `SearchRoutine` returned null IEnumerator → stock `SearchStart` → `StartCoroutine(null)` ("routine is null"). Fix: do not skip `SearchRoutine`; block void `SearchStart` only while apply-suppressed; clear custom-filter guard on stock `SearchStart` / focus-typing; `RecoverAfterFailedApply` on ApplySuggestion catch. (2) **Hangar-free index load:** `GameLoadBootstrap` sole builder; `EditorSearchHook` wait/UI-hook only; no hangar `BuildIfNeeded`; clear stale build lock after scene-host destroy. (3) **1080p readability pass:** larger primary/subtitle/header fonts + brighter contrast colors; branding footer captions slightly larger; named typography constants in `SearchDropdownPanel` / `KoobalWordmarkBuilder`. (4) **Branding footer centering:** empty-query Koobal wordmark no longer drifts left — stretch/center anchors, footer+content MiddleCenter VLGs, DestroyImmediate on rebuild, force layout rebuild after show/width change. (5) **Clicked-suggestion history (additive):** `ApplySuggestion` records display text via `RememberClickedSuggestion` *before* stock apply (so apply exceptions cannot drop it); typed Enter/submit `CommitSearchHistory` path unchanged. **Note:** hangar alignment was briefly labeled **v0.8.5.2** on ModTest; that label is folded back into **v0.8.5.1** (user request) until a new beta is packaged. Local `v0.8.5.2` tag/zip removed to avoid confusion. |
...
As of the v0.8.4.0 core-only re-baseline, the installs are **intentionally split** (user
requested main be left untouched to conserve usage and protect the known-good build):

| Install | Version | Path |
|---------|---------|------|
| Main KSP | **v0.8.3.17** (unchanged) | `GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |
| ModTest | **v0.8.5.1** (SearchStart NRE + hangar-free index + 1080p readability; rollback baseline v0.8.4.0) | `Kerbal Space Program - ModTest/GameData/KoobalSearchEngine/Plugins/KoobalSearchEngine.dll` |

Stable baseline archive: `Desktop/KoobalSearchEngine_v0.8.4.0_CORE_STABLE.zip` (CKAN-style
`GameData/KoobalSearchEngine/`). To promote v0.8.4.0 to main after ModTest validation, copy
the DLL + `README.txt` + `KoobalSearchEngine.version` from ModTest (or the zip) into the main
install's `GameData/KoobalSearchEngine/`.
